<?php
    class Masg{
        ...
        function getAllAsig($asg){
            $sql = "SELECT a.ideqxpr, a.fecent, a.observ, a.fecdev, a.observd, a.numcel, a.opecel, vo.nomval, a.estexp, e.idequ, e.marca, e.modelo, e.serialeq, e.nomred, e.idvtpeq, e.capgb, e.ram, e.procs, e.tipcon, e.contrato, e.valrcont, e.pagequ, vt.nomval AS tpe, vc.nomval AS tpc, pe.idper AS idpent, CONCAT (pe.apeper,' ',pe.nomper) AS pent, pe.ndper AS dpent, vpe.nomval AS tdpent, pe.cargo AS cpent, pr.idper AS idprec, CONCAT (pr.apeper,' ',pr.nomper) AS prec, pr.ndper AS dprec, vpr.nomval AS tdprec, pr.cargo AS cprec, pr.emaper AS eprec, ped.idper AS idpentd, CONCAT (ped.apeper,' ',ped.nomper) AS pentd, ped.ndper AS dpentd, vped.nomval AS tdpentd, ped.cargo AS cpentd, prd.idper AS idprecd, CONCAT (prd.apeper,' ',prd.nomper) AS precd, prd.ndper AS dprecd, vprd.nomval AS tdprecd, prd.cargo AS cprecd, vo.nomval AS operador FROM asignar AS a INNER JOIN equipo AS e ON a.idequ=e.idequ INNER JOIN persona AS pe ON a.idperent=pe.idper INNER JOIN persona AS pr ON a.idperrec=pr.idper LEFT JOIN persona AS ped ON a.idperentd=ped.idper LEFT JOIN persona AS prd ON a.idperrecd=prd.idper INNER JOIN valor AS vpe ON pe.idvtpd=vpe.idval INNER JOIN valor AS vpr ON pr.idvtpd=vpr.idval LEFT JOIN valor AS vped ON ped.idvtpd=vped.idval LEFT JOIN valor AS vprd ON prd.idvtpd=vprd.idval LEFT JOIN valor AS vt ON e.idvtpeq=vt.idval LEFT JOIN valor AS vc ON e.tipcon=vc.idval LEFT JOIN valor AS vo ON a.opecel=vo.idval";
        ...
        function getOne(){
            $sql = "SELECT a.ideqxpr, a.fecent, a.observ, a.fecdev, a.observd, a.numcel, a.opecel, vo.nomval AS operador, a.estexp, e.idequ, e.marca, e.modelo, e.serialeq, e.nomred, e.idvtpeq, e.capgb, e.ram, e.procs, e.tipcon, e.contrato, e.valrcont, e.pagequ, vt.nomval AS tpe, vc.nomval AS tpc, pe.idper AS idpent, CONCAT (pe.apeper,' ',pe.nomper) AS pent, pe.ndper AS dpent, vpe.nomval AS tdpent, pe.cargo AS cpent, pr.idper AS idprec, CONCAT (pr.apeper,' ',pr.nomper) AS prec, pr.ndper AS dprec, vpr.nomval AS tdprec, pr.cargo AS cprec, pr.emaper AS eprec, ped.idper AS idpentd, CONCAT (ped.apeper,' ',ped.nomper) AS pentd, ped.ndper AS dpentd, vped.nomval AS tdpentd, ped.cargo AS cpentd, prd.idper AS idprecd, CONCAT (prd.apeper,' ',prd.nomper) AS precd, prd.ndper AS dprecd, vprd.nomval AS tdprecd, prd.cargo AS cprecd FROM asignar AS a INNER JOIN equipo AS e ON a.idequ=e.idequ INNER JOIN persona AS pe ON a.idperent=pe.idper INNER JOIN persona AS pr ON a.idperrec=pr.idper LEFT JOIN persona AS ped ON a.idperentd=ped.idper LEFT JOIN persona AS prd ON a.idperrecd=prd.idper INNER JOIN valor AS vpe ON pe.idvtpd=vpe.idval INNER JOIN valor AS vpr ON pr.idvtpd=vpr.idval LEFT JOIN valor AS vped ON ped.idvtpd=vped.idval LEFT JOIN valor AS vprd ON prd.idvtpd=vprd.idval LEFT JOIN valor AS vt ON e.idvtpeq=vt.idval LEFT JOIN valor AS vc ON e.tipcon=vc.idval LEFT JOIN valor AS vo ON a.opecel=vo.idval WHERE a.ideqxpr=:ideqxpr";
            $modelo = new conexion();
            $conexion = $modelo->get_conexion();
            $result = $conexion->prepare($sql);
